...
- **Direct** when all dimensions are low and the result is straightforward to inspect.
...
- Rename a label and run the existing test: Direct.
...
|---|---|---|---|
| `QUALIFY` | Inspect context and size the engagement | Mode, rationale, initial unknowns | `INTERVIEW` or Direct `DELIVER` |
...
| `HANDOFF` | Make operation, maintenance, and continuation clear | Handoff notes, next actions, persisted state | `LEARN` |
| `LEARN` | Capture reusable evidence without silently mutating policy | Learning record and any proposal | Terminal or a separately authorized improvement |
...
Use the simplest available storage. Markdown plus structured JSON is sufficient initially. Store public project facts separately from private Maker strategy or customer material. Keep secrets in an approved secret store, never in project state.
...
| Preference | A reusable way the Maker prefers to work or review | Propose a Maker Profile update |
...
Do not infer `ACCEPTED` from silence, a passing test, or an agent report. Direct mode may end with “verified and ready for your review” when explicit acceptance is unnecessary, but it still must not claim the Maker's subjective judgment.
...
Before pausing, persist the current state, contract version, completed evidence, unresolved decisions, risks, and exact next action.
